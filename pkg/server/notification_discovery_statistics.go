...
func HandleDiscoveryStatistics(db *sql.DB, w http.ResponseWriter, r *http.Request) {
	hash := strings.TrimPrefix(r.URL.Path, "/notification-stats-")
	if hash == "" {
		http.Error(w, "Missing or invalid hash", http.StatusBadRequest)
		return
	}

	notifier.CreateStats(db, hash)
